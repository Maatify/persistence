...
{
    public function testTotalPrepareFalse(): void { $this->assertExecutionFailure(new ScriptedPdo([false]), 1); }
    public function testFilteredPrepareFalse(): void { $this->assertExecutionFailure(new ScriptedPdo([ScriptedPdoStatement::count(1), false]), 2); }
    public function testDataPrepareFalse(): void { $this->assertExecutionFailure(new ScriptedPdo([ScriptedPdoStatement::count(1), ScriptedPdoStatement::count(1), false]), 3); }
    public function testTotalBindFalse(): void { $this->assertExecutionFailure(new ScriptedPdo([$this->countStatement(bindResults: [':p' => false])]), 1); }
    public function testFilteredBindFalse(): void { $this->assertExecutionFailure(new ScriptedPdo([ScriptedPdoStatement::count(1), $this->countStatement(bindResults: [':p' => false])]), 2); }
    public function testDataBindFalse(): void { $this->assertExecutionFailure(new ScriptedPdo([ScriptedPdoStatement::count(1), ScriptedPdoStatement::count(1), $this->dataStatement(bindResults: [':p' => false])]), 3); }
    public function testInternalLimitBindFalse(): void { $this->assertExecutionFailure(new ScriptedPdo([ScriptedPdoStatement::count(1), ScriptedPdoStatement::count(1), $this->dataStatement(bindResults: [':__pagination_limit' => false])]), 3); }
    public function testInternalOffsetBindFalse(): void { $this->assertExecutionFailure(new ScriptedPdo([ScriptedPdoStatement::count(1), ScriptedPdoStatement::count(1), $this->dataStatement(bindResults: [':__pagination_offset' => false])]), 3); }
    public function testTotalExecuteFalse(): void { $this->assertExecutionFailure(new ScriptedPdo([$this->countStatement(executeResult: false)]), 1); }
    public function testFilteredExecuteFalse(): void { $this->assertExecutionFailure(new ScriptedPdo([ScriptedPdoStatement::count(1), $this->countStatement(executeResult: false)]), 2); }
    public function testDataExecuteFalse(): void { $this->assertExecutionFailure(new ScriptedPdo([ScriptedPdoStatement::count(1), ScriptedPdoStatement::count(1), $this->dataStatement(executeResult: false)]), 3); }
    ...
    public function testFetchFalseWithNonSuccessSqlstate(): void { $this->assertExecutionFailure(new ScriptedPdo([$this->countStatement(fetchQueue: [false], errorCode: 'HY000')]), 1); }
    public function testInvalidScalarFetch(): void { $this->assertExecutionFailure(new ScriptedPdo([new ScriptedPdoStatement(1, [123], true, '00000')]), 1); }
    public function testNumericKeyRow(): void { $this->assertExecutionFailure(new ScriptedPdo([new ScriptedPdoStatement(1, [[1], false])]), 1); }
    public function testEmptyDataRow(): void { $this->assertExecutionFailure(new ScriptedPdo([ScriptedPdoStatement::count(1), ScriptedPdoStatement::count(1), new ScriptedPdoStatement(1, [[], false])]), 3); }
    public function testCountZeroRows(): void { $this->assertExecutionFailure(new ScriptedPdo([new ScriptedPdoStatement(1, [false])]), 1); }
    public function testCountMultipleRows(): void { $this->assertExecutionFailure(new ScriptedPdo([new ScriptedPdoStatement(1, [['total_count' => 1], ['total_count' => 2], false])]), 1); }
    public function testCountMultipleColumns(): void { $this->assertExecutionFailure(new ScriptedPdo([new ScriptedPdoStatement(2, [['a' => 1, 'b' => 2], false])]), 1); }
